...
from . import config, journal_context, ledger, llm, trades
...
def summarize_cohort(similar: list[dict]) -> dict:
    ...
    if not similar:
        ...
        line = (f"{n} closed trade(s) like this one so far — too few for {config.brand_name()} to state a win rate "
...
def render_report(report: dict) -> str:
    ...
    if report.get("sufficient"):
        ...
        parts.append(f"That is below the {report.get('min_sample', trades.PERF_MIN_SAMPLE)}-trade floor "
                     f"{config.brand_name()} needs before it reports a win rate, so this stays with counts and habits "
