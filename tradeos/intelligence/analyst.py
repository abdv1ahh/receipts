...
def _template_why(item: dict) -> str:
    ...
    if syms:
        ...
        if item.get("has_signal"):
            # The BRAND, from config, not the internal codename. These three strings put
            # "TradeOSS" in front of every reader of the Morning Brief and the News feed, which is
            # exactly the failure CLAUDE.md's rebrand section describes: the display name has two
            # homes and neither is optional.
            parts.append(f"Smart money is already converging on {sym} in {config.brand_name()} "
                         f"signal data, so the reaction here is worth watching.")
    else:
...
def _template_exec(items: list[dict], smart_money_lead: dict | None) -> str:
    ...
    if smart_money_lead and smart_money_lead.get("symbol"):
        parts.append(f"Smart money is also converging on {smart_money_lead['symbol']} in "
                     f"{config.brand_name()} signal data.")
...
def _template_attention(symbol: str, attention: dict, news_items: list) -> str:
    ...
    return (f"{lead}, with no obvious news catalyst in {config.brand_name()} sources; "
            f"a watch it signal, not advice.")
