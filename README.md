...
**Everything binds to `127.0.0.1`.** A bare `docker compose up` is reachable from this machine and
nowhere else — the right default for a laptop on a network you do not control.

**On a real host, serve HTTPS and set `COOKIE_SECURE=true` in `.env`.** Session cookies then carry
the `Secure` flag and the app sends HSTS; `cli preflight` warns for as long as it stays `false`. It
ships `false` because the quickstart is plain HTTP. Do not just move it to a LAN address instead:
**Verify chain** calls `crypto.subtle`, which browsers expose only in a secure context — HTTPS and
`localhost`, nothing else — so on `http://192.168.x.x` the verifier is dead, and a record nobody
can recompute in their own browser is the one thing this project cannot ship.
...
**The tests need no key, no network and no market data.** `make test` is **390 passed, 2 skipped**
on a fresh clone, on a database built from the 37 ordered migrations, and on the instance this was
developed against — the same number in all three, which is the only way the number means anything.
The scoring tests read `tests/fixtures/prices_synthetic.json`, generated by a closed form recorded
inside the file and dated in the past, so they are deterministic and cannot drift with the calendar
or with how much history you have ingested.

**The two skips, and why they are skips rather than failures.** One needs `node`, which is not in
the API image — `make test-js` is the guard that always runs, on the host, and it is the more
important of the two because it is the cross-language check that `chain.py` and `verify.js` still
agree byte for byte. The other reads `docker-compose.yml`, which is not mounted into the container.
Both print their reason. Anything else that skips is a bug.
